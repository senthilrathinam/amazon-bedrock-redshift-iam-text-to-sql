#!/usr/bin/env python3
"""
Fix Bedrock helper to support session tokens.
"""

# Update bedrock_helper.py to include session token
bedrock_helper_fix = '''"""
...
class BedrockHelper:
    ...
    def __init__(self, region_name: str = 'us-east-1'):
        ...
        if session_token:
            ...
            # Permanent credentials (no session token)
            self.bedrock_runtime = boto3.client(
                service_name='bedrock-runtime',
                region_name=region_name,
                aws_access_key_id=os.getenv('AWS_ACCESS_KEY_ID'),
...
with open('src/bedrock/bedrock_helper.py', 'w') as f:
    f.write(bedrock_helper_fix)

print("✅ Updated bedrock_helper.py to support session tokens")
print("Now add AWS_SESSION_TOKEN to your .env file")
